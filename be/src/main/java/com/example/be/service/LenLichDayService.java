...
public class LenLichDayService {
    ...
    public List<LichDay> findLichDayLopByNgay(LopHoc lh, String ngay) {
        return ldRepo.findLichDayLopByDate(lh.getId(), LocalDate.parse(ngay, formatter));
    }
    ...
    public boolean isGiaoVienHasLichDay(GiaoVien giaovien, BuoiHoc bh, LocalDate ngay) {
        Optional<LichDay> ld = ldRepo.findLichDayOfGiaoVienByTime(giaovien.getId(), bh.getId(), ngay);
        return ld.isPresent();
    }
    ...
    public LichDay updateLichDay(LichDay ld) {
        return ldRepo.save(ld);
    }
